...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        //couldnt get what is going on here why in one block you use paginate and not in other one

        //should be same and thats is paginate no need for custom work
        $cuser = User::find($user_id);
        $usertype = '';
        $emergencyJobs = array();
        $noramlJobs = array();
...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            // these lines will be removed after adding store request with validation logic in it
            ...
            //can validate it with validation
            // } else {
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $userlanguage = UserLanguages::where('user_id', '=', $user_id)->pluck('lang_id');
